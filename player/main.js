;zappa.run(function (){
        ...
        urla=window.location.href.split("?");
        url=urla[0];
        query = (urla[1] == undefined) ? "" : urla[1];
        qparapair = query.split("&");
        pars={};
        qparapair.map(function(e){pars[e.split("=")[0]]=e.split("=")[1];});
        window.pars=pars;
        top._url_=url;
        console.log("url parsing complete");
        if (pars.autoexec) {try{eval(pars.autoexec)}catch(e){console.log("autoexec failed")}} // don't put ?s in your autoexec if you want it to work.. for now. encoding might fix it.
...
//            top.skt_connection=this$.connect(null, {
	  ...
	  top.skt_connection=ref$;if (0) {
            ...
            data: function(){
              ...
              case 'log':
                ...
                cmdstr = (function(){
                  ...
                  if (this.data.log==null) this.data.log=[]; // might help compat with frankendis
        ...
        onLoad = function(ssInstance){
          ...
          if (typeof ss.ExecuteCommand === 'function') {
            ...
            ss.ExecuteCommand('set A1 textvalueformat text-html'); // this is a hack that lets me not check for the existance of a textvalueformathash when i am setting textformats from within a sheetfunction -- im forcing it to be generated hopefully
          }
